...
void Renderer::Render(const Scene &scene) {
	...
	for(int j = 0; j < scene.height; ++j) {
		for(int i = 0; i < scene.width; ++i) {
			...
			x = (2 * (static_cast<float>(i) + 0.5f) / static_cast<float>(scene.width) - 1) * imageAspectRatio * scale;
			y = (1 - 2 * (static_cast<float>(j) + 0.5f) / static_cast<float>(scene.height)) * scale;
